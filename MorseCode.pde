//MorseCode.java
// found at http://tanksoftware.com/jtank/src/showsrc.php?src=src/net/jtank/protocol/MorseCode.java
...
public class MorseCode {
	...
	public  String stringToMorse (String text) {
		...
		for (int i = 0; i < text.length(); i++) {
			...
			converted = this.encode(selected);
     ...
     */
	public  String stringFromMorse (String text) {
		...
		for (int i = 0; i < text.length(); i++) {
			...
			if (selected.equalsIgnoreCase(" ") || selected.equalsIgnoreCase("\n")) {
				...
				converted = this.decode(currentString);
		...
		newText = newText + this.decode(currentString);
